...
GenerateManifest.prototype.build = function() {
  ...
  writeFileSync(
    ...
    JSON.stringify(this.filteredManifest(this.manifest)) + '\n');
};

GenerateManifest.prototype.filteredManifest = function(manifest) {
  var filteredManifest = {};

  for(var attribute in manifest) {
    if (attribute !== 'apple' && attribute !== 'ms') {
      filteredManifest[attribute] = manifest[attribute];
    }
  }
